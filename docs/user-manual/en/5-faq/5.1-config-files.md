...
## AutoTier Data Storage

### Storage Directory

Default location: `~/.autotier/`

Customizable location in settings (for cloud sync).

> Legacy data from older CC Switch installations may remain under `~/.cc-switch/`; import it explicitly through AutoTier's migration/import flow rather than using it as the runtime directory.

### Directory Structure

```
~/.autotier/
├── autotier.db      # SQLite database (SSOT)
...
`autotier.db` is a SQLite database that stores:
...
Hermes uses YAML configuration. AutoTier writes MCP servers to `mcp_servers`, writes editable provider entries to `custom_providers`, reads read-only entries from Hermes' `providers` dict, and updates `model.provider` / `model.default` when switching providers.
...
| Field | Description |
|-------|-------------|
| `models.providers` | Provider configuration (mapped to AutoTier's "providers") |
...
AutoTier's priority when modifying configurations:

1. **AutoTier Database** - Single source of truth (SSOT)
...
- CLI tool configuration files (will be backfilled by AutoTier)
- AutoTier's `settings.json`

### Not Recommended to Edit Manually

- `autotier.db` database file
- Backup files
...
Earlier versions migrated the database from JSON files to SQLite:
