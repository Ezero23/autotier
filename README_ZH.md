...
### 🌐 唯一官方网站：**[github.com/Ezero23/autotier/releases](https://github.com/Ezero23/autotier/releases)**
...
Kimi K3 是 Moonshot AI 迄今能力最强的模型，也是全球首个开源 3T 级模型。K3 拥有 2.8T 参数、原生视觉能力与 100 万 Token 上下文，在长程编码、知识工作和推理任务中展现前沿性能。使用 AutoTier，可以在各类 Agent 工具中便捷配置和切换 Kimi。**[点击此处开始使用 Kimi](https://platform.kimi.com?track_id=track-6840233b42274ab4bcfd283e2bdd2aee&aff=cc-switch)**
...
<td>感谢 Fenno.ai 赞助了本项目！Fenno.ai 是一家稳定、高效的 API 中转服务商，目前主要提供 Codex 中转服务，兼容 OpenAI 及 Anthropic 协议，可灵活接入 Codex、Claude Code、OpenCode 等主流编程工具，可稳定支撑千亿 Token/日的企业级调用需求，支持国内及海外主体公对公结算、开票。Fenno.ai 为 AutoTier 的用户提供了专属福利：通过<a href="https://api.fenno.ai/register?redirect=/purchase?tab=subscription%26group=16&aff=P9MR3D3PLCNL">此链接</a>即可订阅 1.99 刀/50 刀额度的 Trial 计划（有效期 7 天），邀请好友最高可享 20% 奖励，多邀多得！</td>
</tr>
...
<td>感谢 AIGoCode 赞助了本项目！AIGoCode 是一个集成了 Claude Code、Codex 以及 Gemini 最新模型的一站式平台，为你提供稳定、高效且高性价比的AI编程服务。本站提供灵活的订阅计划，零封号风险，国内直连，无需魔法，极速响应。AIGoCode 为 AutoTier 的用户提供了特别福利，通过<a href="https://aigocode.app/invite/CC-SWITCH">此链接</a>注册的用户首次充值可以获得额外10%奖励额度！</td>
</tr>

<tr>
<td width="180"><a href="https://aicoding.inc/i/CCSWITCH"><img src="assets/partners/logos/aicoding.jpg" alt="AICoding" width="150"></a></td>
<td>感谢 AICoding 赞助了本项目！AICoding —— 全球大模型 API 超值中转服务！Claude Code 1.9 折，GPT 0.1 折，已为数百家企业提供高性价比 AI 服务。支持 Claude Code、GPT、Gemini 及国内主流模型，企业级高并发、极速开票、7×24 专属技术支持，通过<a href="https://aicoding.inc/i/CCSWITCH">此链接</a> 注册的 AutoTier 用户，首充可享受九折优惠！</td>
</tr>
...
<td>感谢 Right Code 赞助了本项目！Right Code 稳定提供 Claude Code、Codex、Gemini 等模型的中转服务，并可选按量、包月两种计费模式。充值即可开票，企业、团队用户一对一对接。同时为 AutoTier 的用户提供了特别优惠：通过<a href="https://www.rightapi.ai/register?aff=CCSWITCH">此链接</a>注册，每次充值均可获得实付金额5%的按量额度！</td>
</tr>
...
<td>感谢 Cubence 赞助本项目！Cubence 是一家可靠高效的 API 中继服务提供商，提供对 Claude Code、Codex、Gemini 等模型的中继服务，并提供按量、包月等灵活的计费方式。Cubence 为 AutoTier 的用户提供了特别优惠：使用 <a href="https://cubence.com/signup?code=CCSWITCH&source=ccs">此链接</a> 注册，并在充值时输入 "CCSWITCH" 优惠码，每次充值均可享受九折优惠！</td>
</tr>
...
<td>感谢 Crazyrouter 赞助了本项目！Crazyrouter 是一个高性能 AI API 聚合平台——一个 API Key 即可访问 300+ 模型，包括 Claude Code、Codex、Gemini CLI 等。全部模型低至官方定价的 55%，支持自动故障转移、智能路由和无限并发。Crazyrouter 为 AutoTier 用户提供了专属优惠：通过<a href="https://crazyrouter.com/register?aff=OZcm&ref=cc-switch">此链接</a>注册后联系客服即可领取 <strong>$2 免费额度</strong>，首次充值时输入优惠码 `CCSWITCH` 还可获得额外 <strong>30% 奖励额度</strong>！</td>
</tr>
...
**AutoTier** 为你提供一个桌面应用来管理所有支持的 AI 工具。无需手动编辑配置文件，你将获得一个可视化界面，一键将供应商导入应用，一键在不同的供应商之间进行切换，内置 50+ 供应商预设、统一的 MCP, SKILLS 管理以及系统托盘即时切换功能——所有操作都基于可靠的 SQLite 数据库和原子写入机制，保护你的配置不被损坏。
...
- **Deep Link** (`autotier://`) — 通过 URL 一键导入供应商、MCP 服务器、提示词和技能
- 深色 / 浅色 / 跟随系统主题、开机自启、自动更新、原子写入、自动备份、国际化（简中/繁中/英/日）

## 常见问题

<details>
<summary><strong>AutoTier 支持哪些 AI 工具？</strong></summary>

AutoTier 支持八个工具：**Claude Code**、**Claude Desktop**、**Codex**、**Gemini CLI**、**Grok Build**、**OpenCode**、**OpenClaw** 和 **Hermes**。每个工具都有专属的供应商预设和配置管理。
...
AutoTier 使用“通用配置片段”功能，在不同的供应商之间传递 Key 和请求地址之外的通用数据，您可以在“编辑供应商”菜单的“通用配置面板”里，点击“从当前供应商提取”，把所有的通用数据提取到通用配置中，之后在新建“供应商”的时候，只要勾选“应用通用配置”（默认勾选），就会把插件等数据写入到新的供应商配置中。您的所有配置项都会保存在运行本软件的时候，第一次导入的默认供应商里面，不会丢失。
...
当前 AutoTier macOS 构建未签名、未公证，正式 Release 提供 `AutoTier-v0.1.6-macOS.zip`。首次启动可能需要右键选择“打开”，或在 macOS“隐私与安全性”中手动允许。
...
- **数据库**：`~/.autotier/autotier.db`（SQLite — 供应商、MCP、提示词、技能）
- **本地设置**：`~/.autotier/settings.json`（设备级 UI 偏好设置）
- **备份**：`~/.autotier/backups/`（自动轮换，保留最近 10 个）
- **SKILLS**：`~/.autotier/skills/`（默认通过软链接连接到对应应用）
- **技能备份**：`~/.autotier/skill-backups/`（卸载前自动创建，保留最近 20 个）

旧版 CC Switch 数据可能仍位于 `~/.cc-switch/`；请通过 AutoTier 的迁移/导入流程显式导入，不要把它当作运行时默认目录。
...
如果你是从桌面图标启动的，请把它写进 `.desktop` 的 `Exec=` 行（如 `env AUTOTIER_GDK_BACKEND=wayland /path/to/AppImage`），或在会话环境中设置。该变量是通用的：在 tiling Wayland 合成器（sway/Hyprland）下若出现点击失效，可反过来设 `AUTOTIER_GDK_BACKEND=x11`。不设置则保持默认行为。
...
从 [Releases](https://github.com/Ezero23/autotier/releases) 页面下载最新版本的 `AutoTier-v{版本号}-Windows.msi` 安装包或 `AutoTier-v{版本号}-Windows-Portable.zip` 绿色版。

### macOS 用户

**手动下载**

从 [Releases](https://github.com/Ezero23/autotier/releases) 页面下载 `AutoTier-v0.1.6-macOS.zip`；当前未签名版本不发布 DMG。解压得到 `AutoTier.app` 后，将其拖入“应用程序”。首次启动可能需要右键选择“打开”，或在 macOS“隐私与安全性”中手动允许。

### Arch Linux 用户

目前没有官方 AutoTier AUR 包。请从 [Releases](https://github.com/Ezero23/autotier/releases) 下载 Linux AppImage，或手动安装对应发行版的软件包。

### Linux 用户

从 [Releases](https://github.com/Ezero23/autotier/releases) 页面下载最新版本的 Linux 安装包：

- `AutoTier-v{版本号}-Linux.deb`（Debian/Ubuntu）
- `AutoTier-v{版本号}-Linux.rpm`（Fedora/RHEL/openSUSE）
- `AutoTier-v{版本号}-Linux.AppImage`（通用）
...
- **SSOT**（单一事实源）：所有数据存储在 `~/.autotier/autotier.db`（SQLite）
