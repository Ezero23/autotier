...
pub(crate) const TOOL_RESULT_MEDIA_MOVED_MARKER: &str =
    "[autotier: tool result media moved to the following user message]";
pub(crate) const TOOL_RESULT_MEDIA_ATTACHED_MARKER: &str =
    "[autotier: tool result media attached as native media]";
...
pub(crate) fn queue_chat_tool_output_media(
    ...
    pending_media.push(json!({
        "type": "text",
        "text": format!("[autotier: media output of tool call {call_id}]")
    }));
    ...
    match value {
        Value::String(text) => {
            ...
            if should_omit {
                ...
                *text = format!("[autotier: omitted {byte_len} bytes]");
            }
        }
        Value::Array(items) => {
            for item in items {
...
mod tests {
    ...
    fn chat_plan_clamps_residual_base64_inside_json_string_before_serializing() {
        ...
        assert!(plan
            .tool_content
            .contains("[autotier: omitted 20000 bytes]"));
