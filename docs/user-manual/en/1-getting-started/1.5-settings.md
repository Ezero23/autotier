...
This section describes how to configure AutoTier according to your preferences.
...
AutoTier supports four languages:
...
When enabled, AutoTier automatically runs when the system starts.
...
In current AutoTier builds, AutoTier adds **Lightweight Mode** — a **tray-only** running state that minimizes desktop footprint when idle.
...
**How to exit**: Click **Open Main Window** from the tray menu, or trigger AutoTier via deep link / relaunch. The window is **rebuilt on demand**, with state preserved.
...
> **Use case**: If AutoTier runs in the background for long periods and you mainly switch providers via the tray menu, enabling Lightweight Mode significantly reduces memory usage.
...
When enabled, AutoTier automatically syncs the configuration to the VS Code Claude Code extension (writes `primaryApiKey` to `~/.claude/config.json`) when switching providers.
...
Choose the terminal application that AutoTier uses when opening a terminal.
...
The storage location for AutoTier's own data, defaulting to `~/.autotier/`.
...
The exported file name format is `autotier-export-{timestamp}.sql`.
...
Configure AutoTier's outbound HTTP/HTTPS proxy, applicable for scenarios where external API access requires a proxy.
...
When an interval is set, AutoTier automatically backs up the database on schedule. Older backups beyond the retention count are automatically removed.
...
| Remote Directory | Remote storage path (default: `autotier-sync`) |
...
- AutoTier automatically syncs the database to WebDAV at the configured interval
- Sync status is displayed in the panel

#### Remote Snapshot Info

Before downloading, AutoTier displays details about the remote snapshot:
- Protocol version (v2)
...
Available in the current build, the **OAuth Auth Center** (Beta) provides unified management for third-party OAuth credentials. It currently supports three account types:
...
There is no local callback port. Authentication and model requests use AutoTier's global outbound proxy configuration.

The xAI integration does **not** use an OAuth application registered to AutoTier. It reuses the public OAuth client identity and scopes registered for the official Grok CLI (`client_id: b1a00492-073a-47ea-816f-4c329264a828`, including `grok-cli:access`) to obtain account-backed API access.
...
Displays the current AutoTier version number, with support for:
