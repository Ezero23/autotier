# Keep Codex Remote Control and Official Plugins While Using Third-Party APIs: AutoTier Setup Guide

> Applies to AutoTier v3.16.1 and later. This guide is based on the current code, user manual, and v3.16.1 release notes. Screenshots use de-identified sample data and do not include real Access Tokens or API keys.
...
1. In the AutoTier Codex panel, switch to `OpenAI Official`.
2. Start Codex and log in once with an official ChatGPT / Codex account. A Free subscription is enough.
3. Return to AutoTier and enable `Settings -> General -> Codex App Enhancements -> Keep official login when switching third-party providers`.
...
Open AutoTier and switch to the top-level `Codex` tab. First select the `OpenAI Official` provider, or add it from the preset providers if it is missing, and make it the current provider.
...
If it only supports OpenAI Chat Completions, which is common for DeepSeek / Kimi / MiniMax paths, local routing must be enabled so AutoTier can convert Codex Responses requests into Chat Completions requests.
...
After takeover, Codex's live `config.toml` temporarily points to the AutoTier local route. The real third-party API key remains in the AutoTier provider configuration, and is projected into the `experimental_bearer_token` in `config.toml` when providers are switched.
...
- In AutoTier, the current Codex provider is the third-party provider.
...
After `Keep official login when switching third-party providers` is enabled, AutoTier takes the third-party provider API key from the provider configuration and writes it under the current provider in `config.toml`:
...
If the provider uses the Chat Completions protocol, AutoTier local routing adds another conversion layer:

```text
Codex Responses request
        |
AutoTier local route
        |
...
That does not mean model requests are still going to official OpenAI. Actual traffic is determined by the current Codex provider in AutoTier, `config.toml`, and local routing logs.
...
Codex reads the model catalog at startup. Even if AutoTier has generated a new model catalog, a running Codex process may not hot-load it, so restart Codex after editing model mappings.
...
This is expected. Official account information comes from `auth.json`; the actual model provider comes from `config.toml` and the current provider in AutoTier.

**Is a Free subscription really enough?**

Yes. The official account is mainly used to obtain and preserve the official login state required by Codex App. Third-party model requests use the third-party API key configured in AutoTier.

**What should I do if official plugins or mobile remote control still do not work?**

Switch back to `OpenAI Official`, restart Codex, and complete official login once. Then confirm `Settings -> General -> Codex App Enhancements -> Keep official login when switching third-party providers` is enabled in AutoTier before switching back to the third-party provider.
...
Not recommended. AutoTier tries to prevent switching to official providers while local routing takeover is active, because accessing official APIs through a proxy may create account risk. Use official login only to preserve `auth.json`, and route model traffic to third-party providers.
