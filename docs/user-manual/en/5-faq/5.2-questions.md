...
The current AutoTier macOS build is unsigned and not notarized. Download `AutoTier-v0.1.5-macOS.zip` from the [Releases page](https://github.com/Ezero23/autotier/releases); on first launch, you may need to right-click the app and choose **Open**, or allow it in macOS Privacy & Security settings.
...
2. Add AutoTier to your antivirus software's whitelist
...
chmod +x AutoTier-*.AppImage

# If it still fails, try
./AutoTier-*.AppImage --no-sandbox
```
...
**Solution**: Use the dedicated `AUTOTIER_GDK_BACKEND` environment variable to switch back to native Wayland (it is read before GTK init, and the hook never overrides it):

```bash
AUTOTIER_GDK_BACKEND=wayland ./AutoTier-*.AppImage
```

- When launching from a desktop icon, add it to the `.desktop` `Exec=` line (e.g. `env AUTOTIER_GDK_BACKEND=wayland /path/to/AppImage`) or set it in your session environment — otherwise an icon launch won't see the variable.
- The variable is generic: on tiling Wayland compositors (sway/Hyprland) where clicks don't register, set `AUTOTIER_GDK_BACKEND=x11` instead.
...
**Solutions**:
1. Check if the `~/.autotier/` directory exists
2. Restore from backup: `~/.autotier/backups/`
...
**Solutions**:
1. Confirm the file was exported by AutoTier
...
AutoTier uses precise Codex usage parsing from estimation to **precise parsing based on JSONL session logs**, with normalized model names for consistent pricing lookup. New data aligns with official bills. If you still see old estimated data, delete the historical entries or wait for new session data to overwrite them.
...
**Windows**:
- Check taskbar settings to ensure the AutoTier icon is not hidden
...
2. Restart the app
3. Delete `~/.autotier/settings.json` to reset settings
...
3. There is currently no official Homebrew cask; update from the [official GitHub Release](https://github.com/Ezero23/autotier/releases).

## Lightweight Mode

### How to Enter Lightweight Mode?

Toggle "Lightweight Mode" from the system tray menu. The main window closes, and AutoTier runs as a tray-only app. Toggle again or click "Open main window" to exit.
...
Yes. In current AutoTier builds, AutoTier covers all window re-show paths (normal launch, deep links, singleton activation, tray `show_main`, and Lightweight Mode return). Clicking a `autotier://` link **rebuilds the main window on demand** and displays the import confirmation dialog. The first open is slightly slower than normal state (window rebuild required), but subsequent switches return to normal speed.
...
1. Visit [GitHub Issues](https://github.com/Ezero23/autotier/issues)
...
AutoTier stores logs under its application configuration directory. By default this is `.autotier` in your home directory; if you changed the configuration directory in Advanced Settings, use that custom location instead.

When submitting an Issue, attach the files relevant to the problem:

- General, network, or proxy errors: `~/.autotier/logs/autotier.log` and its rotated files
- Application crashes: `~/.autotier/crash.log`, `crash.log.1`, and `crash.log.2`
- The default Windows location is `C:\Users\<username>\.autotier\...`
