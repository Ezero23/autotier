...
AutoTier comes pre-configured with the following GitHub repositories:
...
AutoTier provides powerful search and filter features:
...
- **Automatic backup**: Before deletion, the skill is backed up to `~/.autotier/skill-backups/`
- Removes the skill from all app directories (Claude, Codex, Gemini, OpenCode, Hermes)
- Removes the skill from the SSOT directory (`~/.autotier/skills/`)
...
In current AutoTier builds, Skills support **automatic update detection** and **batch updates** — no more uninstall-and-reinstall.

### Update Detection Mechanism

AutoTier compares installed skills with the remote repository version using **SHA-256 content hashes**. Whenever the remote has any content changes, the corresponding local skill card automatically shows an "Update available" indicator.
...
2. AutoTier batch-downloads all skills with pending updates
...
In current AutoTier builds, the **source storage location** for skills can be switched between two locations:

| Location                 | Description                                                           |
| ------------------------ | --------------------------------------------------------------------- |
| **AutoTier built-in**   | Default location `~/.autotier/skills/`, managed by AutoTier         |
| **`~/.agents/skills`**   | A shared directory conforming to community agent tool conventions, better for cross-tool collaboration |

### How to Switch

Select the target storage location from the settings or management menu in the Skills panel. The switch **does not lose skill state** — AutoTier smoothly migrates existing skills to the new location.
...
The current AutoTier build integrates **skills.sh** public registry search so you can discover community skills directly inside AutoTier.
...
The current AutoTier build also fixes broken link and empty description handling for skills.sh, so community skill metadata is displayed more reliably.
...
- AutoTier has not finished the latest scan
