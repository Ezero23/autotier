...
describe("OpenCodeFormFields", () => {
  it("surfaces existing provider headers", () => {
    ...
    expect(
      screen.getByDisplayValue("https://autotier.app"),
    ).toBeInTheDocument();
    expect(screen.getByDisplayValue("X-Title")).toBeInTheDocument();
    expect(screen.getByDisplayValue("AutoTier")).toBeInTheDocument();
  });

  it("updates provider headers", () => {
    const onHeadersChange = vi.fn();
    renderOpenCodeForm({
      headers: { "X-Title": "AutoTier" },
      onHeadersChange,
    });

    fireEvent.change(screen.getByDisplayValue("AutoTier"), {
