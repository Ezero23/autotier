# 在 Codex 中用 Kimi：AutoTier 本地路由攻略

> 适用版本：AutoTier 3.16.5 及附近版本。本文根据仓库内文档与代码整理，并用 Kimi 作为 OpenAI Chat Completions 兼容接口的示例。截图来自当前前端界面，使用去敏示例数据生成，避免泄露真实 API Key 或账户余额。
...
Kimi For Coding 官方目前支持的第三方工具是 Claude Code、Roo Code 这类兼容 Anthropic 协议的编程 Agent，并没有覆盖 Codex。所以想在 Codex 里用 Kimi，需要一层协议转换——这正是 AutoTier 本地路由做的事。
...
Kimi 的 API Key 有两个来源，对应 AutoTier 里两个不同的内置预设：
...
接管后，AutoTier 会把 Codex 的 live 配置指向本机路由，并用占位符管理认证。真实 Kimi Key 仍保存在 AutoTier 的 Provider 配置里，由本地路由在转发时注入，不需要你把 Key 暴露给 Codex live 配置。

## 第三步：切换供应商并重启 Codex

回到 Codex 供应商列表，点击 Kimi 供应商的 `启用`。如果看到 `需要路由` 标记，说明这个供应商必须在路由运行时使用；没有启动路由时，AutoTier 会弹出“需要路由服务才能正常使用”的提示。
...
Kimi、DeepSeek、MiniMax、SiliconFlow 等常见 Chat 格式供应商在 AutoTier 里已有预设，优先用预设即可。只有预设里没有的供应商，才需要选择自定义配置；这时按对方文档填 API Key、base URL 和模型，并把 `高级选项` 里的 `上游格式` 选为 `Chat Completions（需开启路由）`。

如果上游直接支持 OpenAI Responses API，把 `上游格式` 选为 `Responses` 即可；这时 AutoTier 按 Responses 直连，不做 Chat 转换。
