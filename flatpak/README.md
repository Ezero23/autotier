...
This directory contains the Flatpak manifest (`com.ezero.autotier.yml`) for AutoTier, used to convert the generated `.deb` artifact into an installable `.flatpak` package via CI or local builds.
...
```bash
cp "$(find src-tauri/target/release/bundle -name '*.deb' | head -n 1)" flatpak/autotier.deb
```

3) Build the local Flatpak repository and export the `.flatpak`:

```bash
flatpak-builder --force-clean --user --disable-cache --repo flatpak-repo flatpak-build flatpak/com.ezero.autotier.yml
flatpak build-bundle --runtime-repo=https://flathub.org/repo/flathub.flatpakrepo flatpak-repo AutoTier-Linux.flatpak com.ezero.autotier
```

4) Install and run:

```bash
flatpak install --user ./AutoTier-Linux.flatpak
flatpak run com.ezero.autotier
```
...
If you prefer minimal permissions (e.g., for Flathub submission or security concerns), you can replace `--filesystem=home` in `flatpak/com.ezero.autotier.yml` with more precise grants:

```yaml
  - --filesystem=~/.autotier:create
...
If you plan to publish on Flathub or want stricter permission control, adjust the `finish-args` in `flatpak/com.ezero.autotier.yml` accordingly.
