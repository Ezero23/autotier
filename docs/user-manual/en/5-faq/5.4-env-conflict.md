...
AutoTier automatically detects conflicts between system environment variables and app configurations, preventing configurations from being unexpectedly overridden.
...
Found X environment variables that may conflict with AutoTier configuration
[Expand] [Dismiss]
```
...
3. AutoTier will automatically back up and remove the selected variables

### Automatic Backup

A backup is automatically created before removal:

- Backup location: `~/.autotier/env-backups/`
...
If you prefer not to use AutoTier to remove variables, you can handle them manually:
...
- AutoTier provider configurations being overridden
- API requests being sent to the wrong endpoint
- Using the wrong API key

## Best Practices

1. **Use AutoTier to manage configurations**: Avoid setting API keys in system environment variables
...
1. Find the backup file: `~/.autotier/env-backups/`
