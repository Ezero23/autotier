...
"#
    ...
    let auth_value: serde_json::Value =
        read_json_file(&autotier_lib::get_codex_auth_path()).expect("read auth.json");
    assert_eq!(
    ...
    let config_text =
        std::fs::read_to_string(autotier_lib::get_codex_config_path()).expect("read config.toml");
    assert!(
...
"#;
    ...
    let auth_after_switch: serde_json::Value =
        read_json_file(&autotier_lib::get_codex_auth_path()).expect("read auth after switch");
    assert_eq!(
    ...
    let config_after_switch =
        std::fs::read_to_string(autotier_lib::get_codex_config_path()).expect("read config");
    assert!(
    ...
    let auth_after_takeover: serde_json::Value =
        read_json_file(&autotier_lib::get_codex_auth_path()).expect("read auth after takeover");
    assert_eq!(
    ...
    let restored_auth: serde_json::Value =
        read_json_file(&autotier_lib::get_codex_auth_path()).expect("read restored auth");
    assert_eq!(
        restored_auth, oauth_auth,
        "disabling takeover should restore without replacing OAuth auth.json"
    );

    let restored_config = std::fs::read_to_string(autotier_lib::get_codex_config_path())
...
fn provider_service_reswitch_current_official_keeps_live_auth() {
    ...
    let auth_value: serde_json::Value =
        read_json_file(&autotier_lib::get_codex_auth_path()).expect("auth.json must survive");
    assert_eq!(
        auth_value.get("OPENAI_API_KEY").and_then(|v| v.as_str()),
        Some("residue-key"),
...
fn read_codex_live_settings_tolerates_missing_auth_when_config_file_exists() {
    ...
    assert!(
        autotier_lib::read_codex_live_settings().is_err(),
    ...
    let live = autotier_lib::read_codex_live_settings()
...
fn reapply_codex_official_live_resyncs_mcp_servers() {
    ...
    let live = std::fs::read_to_string(autotier_lib::get_codex_config_path())
    ...
    let reapplied =
        autotier_lib::reapply_current_codex_official_live(&state).expect("reapply official live");
    assert!(
        reapplied,
...
fn reapply_codex_official_live_projects_mcp_despite_broken_claude_json() {
    ...
    let claude_json = autotier_lib::get_claude_mcp_path();
    std::fs::write(&claude_json, "{ not valid json").expect("seed broken claude json");

    let reapplied = autotier_lib::reapply_current_codex_official_live(&state)
...
fn sync_all_enabled_reports_broken_app_but_projects_the_rest() {
    ...
    let err = autotier_lib::McpService::sync_all_enabled(&state)
...
fn provider_service_switch_codex_official_accounts_write_auth_json() {
    ...
    let auth_b: serde_json::Value =
        read_json_file(&autotier_lib::get_codex_auth_path()).expect("read auth B");
    assert_eq!(
        auth_b
            .pointer("/tokens/access_token")
            .and_then(|v| v.as_str()),
    ...
    let auth_a: serde_json::Value =
        read_json_file(&autotier_lib::get_codex_auth_path()).expect("read auth A");
    assert_eq!(
        auth_a
            .pointer("/tokens/access_token")
            .and_then(|v| v.as_str()),
...
"#;
    ...
    let live_after = std::fs::read_to_string(autotier_lib::get_codex_config_path())
        .expect("read config.toml after switch");
    assert!(
