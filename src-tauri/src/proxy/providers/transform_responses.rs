...
pub(crate) const TOOL_RESULT_ERROR_MARKER: &str = "[autotier:tool-result-error]";
...
pub fn anthropic_to_responses(
    ...
    //   （AutoTier 的 transform 当前是"条件写入"，可能产生缺失）
    // - service_tier: 仅在 FAST mode 开启时写入 "priority"
    //   （与 OpenAI 官方 codex-rs 当前请求结构保持一致）
    // - stream: 必须永远 true（codex-rs 硬编码 true，且 AutoTier 的
