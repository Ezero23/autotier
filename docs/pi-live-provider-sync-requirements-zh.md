...
Pi 会读取 `~/.pi/agent/models.json` 中的 `providers`，并把同名显式供应商配置与自己的内置供应商、内置模型合并。当前 AutoTier 会跳过 `anthropic`、`openai`、`deepseek` 等 Pi 内置供应商 ID，导致 Pi 已经加载了显式配置，但 AutoTier 页面没有对应的已启用卡片，也无法正常移除、编辑或重新启用。

这类供应商是否属于 AutoTier 可管理范围，应只由配置来源决定，不能再由供应商 ID 或认证类型决定。
...
| 显式 provider 同步为 AutoTier 卡片 | 显式 provider 同步为 AutoTier 卡片 |
...
- provider 只存在于 AutoTier 数据库时，卡片保留并显示为未启用。
- 已启用卡片沿用 OpenCode 的蓝色高亮。
- 外部修改 `models.json` 后，刷新供应商页面即自动同步，不要求二次确认。
- 不再出现“不是 AutoTier 管理的值”或“配置刚发生变化，请确认后重试”等 ownership 冲突提示。
...
- `models.json.providers` 中的显式配置由 AutoTier 管理。
- Pi `/login` 写入 `auth.json` 的凭证始终由 Pi 管理，无论其类型为 OAuth 还是 API Key。
- AutoTier 不读取、复制、修改或删除 `auth.json` 中的凭证。
- AutoTier 不刷新 OAuth Token。
- 只有 `auth.json` 凭证、没有 `models.json.providers` 节点的供应商，不生成 AutoTier 卡片。
- 环境变量中的凭证不导入 AutoTier。
- 同一供应商同时存在显式配置和 Pi 登录时，AutoTier 只管理显式配置。
- 移除显式配置不退出 Pi 登录，也不删除 `auth.json`。
- 用户在 AutoTier 中创建的 API Key 供应商仍由 AutoTier 保存并写入 `models.json`；用户通过 Pi `/login` 保存的 API Key 仍属于 Pi 原生认证状态。

## 启用与移除

### 启用

- 将 AutoTier 数据库中保存的完整配置写入 `models.json.providers.<providerId>`。
- 成功后卡片变为蓝色已启用状态。
...
- 不把 Pi 的内置模型复制到 AutoTier 数据库或 `models.json`。
...
6. 在 AutoTier 外部新增、修改或删除 provider，刷新页面后自动同步。
