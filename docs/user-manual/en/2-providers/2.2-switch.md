...
2. AutoTier updates the configuration file
...
1. Right-click the AutoTier icon in the system tray
...
In current AutoTier builds, the tray menu is refactored from a flat list into **per-app submenus**, with a dedicated submenu for each app:
...
When switching providers, AutoTier modifies the following files:
