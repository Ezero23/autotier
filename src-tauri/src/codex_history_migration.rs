...
use crate::codex_config::{
    get_codex_config_dir, read_codex_config_text, AUTOTIER_CODEX_MODEL_PROVIDER_ID,
};
...
const LEGACY_CODEX_MODEL_PROVIDER_ID: &str = "ccswitch";
// If a Codex preset ever used a legacy temporary routing key, keep that old key here
// so local history can be bucketed under the current custom provider id.
const LEGACY_CODEX_MODEL_PROVIDER_IDS: &[&str] = &[
    LEGACY_CODEX_MODEL_PROVIDER_ID,
    "aicodemirror",
    "aicoding",
    "aigocode",
    "aihubmix",
    ...
    if source_provider_ids.is_empty() {
        crate::settings::mark_codex_third_party_history_provider_bucket_migrated(
            CodexThirdPartyHistoryProviderBucketMigration {
                ...
                target_provider_id: AUTOTIER_CODEX_MODEL_PROVIDER_ID.to_string(),
...
/// 迁移前 jsonl / state DB 均备份到 `~/.autotier/backups/codex-official-history-unify-v1/`。
    ...
    config_text
        ...
        .and_then(|doc| {
            doc.get("model_provider")
                ...
                .map(|id| id.trim() == AUTOTIER_CODEX_MODEL_PROVIDER_ID)
        })
        .unwrap_or(false)
}
...
fn rewrite_codex_session_meta_line_for_restore(
    ...
    if payload.get("model_provider")?.as_str()? != AUTOTIER_CODEX_MODEL_PROVIDER_ID {
        return None;
    }
...
fn restore_codex_state_db_official_threads(
    ...
    for chunk in ids.chunks(STATE_DB_ID_CHUNK) {
        ...
        values.push(AUTOTIER_CODEX_MODEL_PROVIDER_ID.to_string());
...
fn collect_source_model_provider_ids(db: &Database) -> Result<BTreeSet<String>, AppError> {
    ...
    for provider in providers.values() {
        ...
        insert_known_legacy_source_id(&mut ids, &provider.id);
        ...
        for provider_id in trusted_legacy_codex_model_provider_ids_from_config(config_text) {
            insert_known_legacy_source_id(&mut ids, &provider_id);
        }
...
fn insert_known_legacy_source_id(ids: &mut BTreeSet<String>, provider_id: &str) {
    let trimmed = provider_id.trim();
    if is_known_legacy_codex_model_provider_id(trimmed) {
...
fn is_known_legacy_codex_model_provider_id(provider_id: &str) -> bool {
    LEGACY_CODEX_MODEL_PROVIDER_IDS
        .iter()
...
fn legacy_codex_model_provider_id_from_normalized_config(config_text: &str) -> Option<String> {
    ...
    let provider_id = doc
        ...
        .and_then(|item| item.as_str())
        .map(str::trim)?;
    if provider_id != AUTOTIER_CODEX_MODEL_PROVIDER_ID
        && provider_id != LEGACY_CODEX_MODEL_PROVIDER_ID
    {
        return None;
    }
...
fn normalized_legacy_codex_provider_name(name: &str) -> Option<&'static str> {
    if is_known_legacy_codex_model_provider_id(name) {
        return LEGACY_CODEX_MODEL_PROVIDER_IDS
            .iter()
            .copied()
...
fn insert_trusted_legacy_config_model_provider_id(
    ...
    if provider_id.is_empty()
        || !is_known_legacy_codex_model_provider_id(provider_id)
...
fn migrate_provider_config_template_to_custom(
    ...
    if let Some(source_provider_id) = source_provider_id_to_move {
        ...
        model_providers[AUTOTIER_CODEX_MODEL_PROVIDER_ID] = provider_table;
        changed = true;
    }
    ...
    for profile_key in profile_keys {
        ...
        if references_legacy {
            profile_table.insert(
                "model_provider",
                toml_edit::value(AUTOTIER_CODEX_MODEL_PROVIDER_ID),
            );
            changed = true;
        }
    }
...
fn rewrite_codex_session_meta_line(
    ...
    payload.insert(
        "model_provider".to_string(),
        Value::String(AUTOTIER_CODEX_MODEL_PROVIDER_ID.to_string()),
    );
    serde_json::to_string(&value).ok()
}
...
"#
    ...
    #[test]
    fn skips_custom_category_unknown_provider_when_created_by_legacy_install() {
        let db = Database::memory().expect("memory db");
        let mut provider = Provider::with_id(
            "generated-uuid".to_string(),
            "Manual Relay".to_string(),
