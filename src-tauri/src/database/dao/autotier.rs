    ...
    pub fn autotier_upsert_decision(&self, row: &AutotierDecisionRow) -> Result<(), AppError> {
        ...
        conn.execute(
            "INSERT INTO autotier_routing_decisions (
                ...
                vision_fallback_applied, vision_describe_input_tokens,
            ...
            params![
                ...
                row.vision_fallback_applied,
                row.vision_describe_input_tokens,
                row.vision_describe_output_tokens,
    ...
    pub fn autotier_finalize_decision(
        ...
        let result = conn.execute(
            "UPDATE autotier_routing_decisions SET
                ...
                vision_fallback_applied = COALESCE(?5, vision_fallback_applied),
                vision_describe_input_tokens = COALESCE(?6, vision_describe_input_tokens),
                vision_describe_output_tokens = COALESCE(?7, vision_describe_output_tokens),
                upstream_message_id = COALESCE(?8, upstream_message_id),
                usage_request_id = COALESCE(?9, usage_request_id),
                actual_input_tokens = COALESCE(?10, actual_input_tokens),
                actual_output_tokens = COALESCE(?11, actual_output_tokens),
                actual_cache_read_tokens = COALESCE(?12, actual_cache_read_tokens),
                actual_cache_write_5m_tokens = COALESCE(?13, actual_cache_write_5m_tokens),
                actual_cache_write_1h_tokens = COALESCE(?14, actual_cache_write_1h_tokens),
                actual_cost_usd = COALESCE(?15, actual_cost_usd),
                status_code = COALESCE(?16, status_code),
                outcome = COALESCE(?17, outcome),
                retry_count = COALESCE(?18, retry_count),
                fallback_count = COALESCE(?19, fallback_count),
                error_code = COALESCE(?20, error_code),
                is_complete = CASE WHEN ?21 = 1 THEN 1 ELSE is_complete END
             WHERE decision_id = ?1",
            params![
                params.decision_id,
                params.completed_at,
                ...
                params.vision_fallback_applied,
                params.vision_describe_input_tokens,
                params.vision_describe_output_tokens,
        ...
        let row = stmt
            .query_row([decision_id], |row| {
                Ok(AutotierDecisionRow {
                    ...
                    vision_fallback_applied: row.get(16)?,
                    vision_describe_input_tokens: row.get(17)?,
                    vision_describe_output_tokens: row.get(18)?,
                    upstream_message_id: row.get(19)?,
                    usage_request_id: row.get(20)?,
                    complexity_score: row.get(21)?,
                    confidence: row.get(22)?,
                    reason_codes_json: row.get(23)?,
                    unsafe_reasons_json: row.get(24)?,
                    safe_to_execute: row.get(25)?,
                    feature_json: row.get(26)?,
                    feature_version: row.get(27)?,
                    classifier_version: row.get(28)?,
                    policy_version: row.get(29)?,
                    actual_input_tokens: row.get(30)?,
                    actual_output_tokens: row.get(31)?,
                    actual_cache_read_tokens: row.get(32)?,
                    actual_cache_write_5m_tokens: row.get(33)?,
                    actual_cache_write_1h_tokens: row.get(34)?,
                    actual_cost_usd: row.get(35)?,
                    candidate_cost_low_usd: row.get(36)?,
                    candidate_cost_base_usd: row.get(37)?,
                    candidate_cost_high_usd: row.get(38)?,
                    cost_assumptions_json: row.get(39)?,
                    status_code: row.get(40)?,
                    outcome: row.get(41)?,
                    retry_count: row.get(42)?,
                    fallback_count: row.get(43)?,
                    is_complete: row.get(44)?,
                    error_code: row.get(45)?,
                })
            })
            .optional()
            .map_err(|e| AppError::Database(e.to_string()))?;
        ...
        let row = conn
            .query_row(
                "SELECT mode, advisory_candidate, retention_days, raw_prompt_opt_in,
                        vision_copilot_enabled, vision_copilot_model, vision_text_only_models,
                        classifier_version, feature_version, policy_version, updated_at
                 FROM autotier_routing_config
                 WHERE id = 1",
                [],
                |row| {
                    let text_only_models: String = row.get(6)?;
                    Ok(AutotierRoutingConfigDto {
                        mode: row.get(0)?,
                        advisory_candidate: row.get(1)?,
                        retention_days: row.get(2)?,
                        raw_prompt_opt_in: row.get(3)?,
                        vision_copilot_enabled: row.get(4)?,
                        vision_copilot_model: row.get(5)?,
                        vision_text_only_models: serde_json::from_str(&text_only_models)
                            .unwrap_or_default(),
    ...
    pub fn autotier_set_config(&self, config: &AutotierRoutingConfigDto) -> Result<(), AppError> {
        ...
        conn.execute(
            ...
            ON CONFLICT(id) DO UPDATE SET
                ...
                vision_copilot_enabled = excluded.vision_copilot_enabled,
                vision_copilot_model = excluded.vision_copilot_model,
                vision_text_only_models = excluded.vision_text_only_models,
            ...
            params![
                ...
                config.vision_copilot_enabled,
                config.vision_copilot_model,
                serde_json::to_string(&config.vision_text_only_models)
                    .map_err(|e| AppError::Database(e.to_string()))?,
...
mod tests {
    ...
    #[test]
    fn migration_to_v20_is_idempotent() {
        // 验证：迁移完成后 autotier 表存在且可写，版本号停在 20
