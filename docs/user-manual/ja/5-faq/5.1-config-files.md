...
```
~/.autotier/
├── autotier.db      # SQLite データベース（SSOT）
...
`autotier.db` は SQLite データベースで、以下を保存しています：
...
Hermes は YAML 設定を使用します。AutoTier は MCP サーバーを `mcp_servers` に書き込み、編集可能なプロバイダーエントリを `custom_providers` に書き込み、Hermes の `providers` dict にある読み取り専用エントリを読み取り、プロバイダー切り替え時に `model.provider` / `model.default` を更新します。
...
| フィールド | 説明 |
|------|------|
| `models.providers` | プロバイダー設定（AutoTier の「プロバイダー」にマッピング） |
| `env` | 環境変数設定 |
...
AutoTier が設定を変更する際の優先順位：

1. **AutoTier データベース** - 単一事実源 (SSOT)
...
- CLI ツールの設定ファイル（AutoTier がバックフィルする）
- AutoTier の `settings.json`

### 手動編集を推奨しないもの

- `autotier.db` データベースファイル
- バックアップファイル
...
以前のバージョンで JSON ファイルから SQLite に移行しました：
