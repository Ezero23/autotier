...
///
/// Input (from AutoTier UI / database):
/// ```json
...
/// `providers:` dict. AutoTier treats those as read-only — writes have to
...
/// - `custom_providers:` list entries (writable by AutoTier)
...
/// keeping AutoTier free to edit it. Models are denormalized from the YAML
...
/// list — i.e. it is a read-only overlay AutoTier must not touch.
...
pub fn set_provider(
        ...
        .find(|p| p.get("name").and_then(|n| n.as_str()) == Some(name))
    {
        ...
        // this merge, a AutoTier edit to an unrelated field would silently
...
// AutoTier fills that gap by reading/writing the whole file as a markdown
...
mod tests {
    ...
    ///
    /// Saves and restores `AUTOTIER_TEST_HOME` to avoid interfering with
    /// parallel tests in other modules.
    fn with_test_home<T>(test_fn: impl FnOnce() -> T) -> T {
        let _guard = test_guard();
        let tmp = tempfile::tempdir().unwrap();
        let old_test_home = std::env::var_os("AUTOTIER_TEST_HOME");
...
";
    ...
    fn set_provider_preserves_unknown_fields_on_update() {
        ...
        // Hermes Web UI; a later AutoTier edit must not strip them — set_provider
...
";
    ...
    fn set_memory_enabled_preserves_other_fields() {
        ...
        // a AutoTier toggle would silently wipe those fields.
        with_test_home(|| {
            let yaml = "\
memory:
