...
fn handle_deeplink_url(
    ...
    let has_autotier_scheme = url::Url::parse(url_str)
        .map(|url| url.scheme() == crate::deeplink::DEEPLINK_SCHEME)
        .unwrap_or(false);
    if !has_autotier_scheme {
        return false;
    }
...
pub fn run() {
    // 设置 panic hook，在应用崩溃时记录日志到 <app_config_dir>/crash.log（默认 ~/.autotier/crash.log）
    ...
    let builder = builder
        ...
        .setup(|app| {
            ...
            // 初始化日志（输出到 <app_config_dir>/logs/autotier.log）
            {
                ...
                log::info!("=== AutoTier v{} started ===", env!("CARGO_PKG_VERSION"));
            }
            ...
            //
            // 捕获首次运行快照：所有全新装用户都会看到欢迎弹窗介绍 AutoTier 的工作方式。
            ...
            #[cfg(any(target_os = "linux", all(debug_assertions, windows)))]
            {
                #[cfg(target_os = "linux")]
                {
                    ...
                    // tauri-plugin-deep-link writes to: ~/.local/share/com.ezero.autotier/applications/autotier-handler.desktop
                ...
                move |event| {
                    ...
                    for (i, url) in urls.iter().enumerate() {
                        ...
                        if handle_deeplink_url(&app_handle, url_str, true, "on_open_url") {
                            break; // Process only first autotier:// URL
                        }
                    }
                }
            });
    ...
    app.run(|app_handle, event| {
        ...
        #[cfg(target_os = "macos")]
        {
            match event {
                ...
                // 处理通过自定义 URL 协议触发的打开事件（例如 autotier://...）
...
fn show_migration_error_dialog(app: &tauri::AppHandle, error: &str) -> bool {
    ...
    let message = if is_chinese_locale() {
        ...
        format!(
            ...
            Consider rolling back to an older AutoTier version.\n\n\
...
fn show_database_init_error_dialog(
    ...
    let message = if is_chinese_locale() {
        ...
        format!(
            ...
            Suggestions:\n\
            1) Back up the entire config directory (including autotier.db)\n\
            2) If you see “database version is newer”, please upgrade AutoTier\n\
