...
App routing means letting AutoTier route a specific application's API requests through the local routing service.
...
When routing is enabled, AutoTier modifies the app's configuration file to point the API endpoint to the local routing service.
...
When disabling routing, AutoTier will:
...
Before enabling routing, AutoTier backs up the original configuration and restores it when disabled.
