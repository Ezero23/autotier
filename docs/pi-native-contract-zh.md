...
> 验证原则：开发和验收使用当时最新发布的 Pi；AutoTier 不绑定某个 Pi 版本。

这份文档记录 AutoTier 当前实际消费的 Pi 原生契约。它不是 Pi 配置格式的完整镜像，也不承诺代理、OAuth 或所有兼容字段。实现和测试只覆盖前端已经提供的能力。
...
已有配置中的其他字段原样保留。供应商是否可管理只取决于节点是否显式存在于 `models.json.providers`：`anthropic`、`openai`、`deepseek` 等 Pi 内置 ID，以及带有未知字段的节点，都按普通显式配置同步。AutoTier 不把 Pi 运行时合并出的内置模型复制回配置，也不解析或执行 `apiKey`、Header 中的环境变量和命令表达式。请求仍由 Pi 自己发出。
...
用量查询脚本属于 AutoTier 元数据，只更新数据库，不重写 `models.json`。

### 并发与外部修改

供应商标识是成员身份：`models.json.providers` 中存在该标识即为已启用。AutoTier 每次进入列表及应用启动时同步全部显式节点，不按内置 ID、认证字段或模型完整度过滤。原生配置发生修改后，以原生内容更新同 ID 的已保存档案；原生删除只改变启用状态。移除前保存目标节点的最新内容，数据库中的卡片和完整配置继续保留。
...
全局会话页只枚举绝对 `sessionDir`、`~` 路径或 Pi 默认目录。相对 `sessionDir` 依赖启动 Pi 时的项目工作目录，AutoTier 没有可靠上下文，因此明确显示“需要项目上下文”，不会猜测目录。
